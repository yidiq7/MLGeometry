...
def train_optax(model: Any,
    ...
    if params is None:
        ...
        if verbose:
            print(f"Initializing parameters for input dimension {input_dim}...")
    ...
    if verbose:
        print(f"Starting training with {epochs} epochs, {num_batches} batches/epoch...")
    ...
    if verbose:
        print(f"Training finished in {total_time:.2f}s. Final Loss: {avg_loss:.5f}")
...
def train_lbfgs(model: Any,
    ...
    if verbose:
        ...
        print(f"Starting L-BFGS training ({mode})...")
    ...
    if verbose:
        ...
        print(f"Initial Loss: {state.value:.5f}")
        
        for i in range(1, max_iter + 1):
            params, state = step(params, state)
            print(f"Iteration {i}: Loss = {state.value:.5f}")
        ...
        print(f"L-BFGS finished in {time.time() - start_time:.2f}s. Final Loss: {final_loss:.5f}")
